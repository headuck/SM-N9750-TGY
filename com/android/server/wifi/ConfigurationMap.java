...
public class ConfigurationMap {
    ...
    public WifiConfiguration remove(int netID) {
        ...
        while (true) {
            if (scanResultMatchInfoEntries.hasNext()) {
                if (((WifiConfiguration) scanResultMatchInfoEntries.next().getValue()).networkId == netID) {
    ...
    public WifiConfiguration getByScanResultForCurrentUser(ScanResult scanResult) {
        return getByScanResultForCurrentUser(scanResult, false);
    }

    public WifiConfiguration getByScanResultForCurrentUser(ScanResult scanResult, boolean onlyPasspoint) {
        return this.mScanResultMatchInfoMapForCurrentUser.get(ScanResultMatchInfo.fromScanResult(scanResult, onlyPasspoint));
    }
