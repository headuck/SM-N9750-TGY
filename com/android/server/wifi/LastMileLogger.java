...
public class LastMileLogger {
    ...
    private void enableTracing() {
        if (!ensureFailSafeIsArmed()) {
            this.mLog.mo2097wC("Failed to arm fail-safe.");
            return;
        }
        try {
            FileUtils.stringToFile(this.mEventEnablePath, "1");
        } catch (IOException e) {
            this.mLog.warn("Failed to start event tracing: %").mo2073r(e.getMessage()).flush();
        }
    }
    ...
    private void disableTracing() {
        try {
            ...
            this.mLog.warn("Failed to stop event tracing: %").mo2073r(e.getMessage()).flush();
        }
    }
    ...
    private byte[] readTrace() {
        try {
            ...
            this.mLog.warn("Failed to read event trace: %").mo2073r(e.getMessage()).flush();
            return new byte[0];
        }
    }
    ...
    private boolean ensureFailSafeIsArmed() {
        ...
        try {
            ...
            this.mLog.warn("Failed to open free_buffer pseudo-file: %").mo2073r(e.getMessage()).flush();
            return false;
        }
    }
