...
public class WifiCandidates {
    ...
    public static class ScoredCandidate {
        public static final ScoredCandidate NONE = new ScoredCandidate(Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, (Candidate) null);
    ...
    public static class Key {
        ...
        public int hashCode() {
            return Objects.hash(new Object[]{this.matchInfo, this.bssid, Integer.valueOf(this.networkId)});
        }
    }
    ...
    public boolean add(ScanDetail scanDetail, WifiConfiguration config, int evaluatorId, int evaluatorScore, double lastSelectionWeightBetweenZeroAndOne) {
        ...
        try {
            ...
            ScanResultMatchInfo key2 = ScanResultMatchInfo.fromScanResult(scanResult, config.isPasspoint());
            if (!key1.equals(key2)) {
                return failure(key1, key2);
            }
            Key key = new Key(key1, bssid, wifiConfiguration.networkId);
            CandidateImpl old = this.mCandidates.get(key);
            if (old == null) {
                int i2 = evaluatorScore;
            } else if (i < old.evaluatorId) {
                return failure(new Object[0]);
            } else {
                if (i > old.evaluatorId || evaluatorScore <= old.evaluatorScore) {
                    return false;
                }
                remove(old);
            }
            ...
            perBssid.setNetworkConfigId(wifiConfiguration.networkId);
            double min = Math.min(Math.max(lastSelectionWeightBetweenZeroAndOne, 0.0d), 1.0d);
            CandidateImpl candidateImpl = old;
            ScanResultMatchInfo scanResultMatchInfo = key2;
            double d = min;
            ScanResultMatchInfo scanResultMatchInfo2 = key1;
            MacAddress macAddress = bssid;
            CandidateImpl candidate = new CandidateImpl(key, scanDetail, config, evaluatorId, evaluatorScore, perBssid, d, wifiConfiguration.networkId == this.mCurrentNetworkId, bssid.equals(this.mCurrentBssid));
            this.mCandidates.put(key, candidate);
            return true;
        } catch (RuntimeException e) {
