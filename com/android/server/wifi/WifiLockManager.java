...
public class WifiLockManager {
    ...
    private boolean canActivateLowLatencyLock(int ignoreMask) {
        return canActivateLowLatencyLock(ignoreMask, (UidRec) null);
    }

    /* access modifiers changed from: private */
    public boolean canActivateLowLatencyLock() {
        return canActivateLowLatencyLock(0, (UidRec) null);
    }
            ...
            public void onUidImportance(int uid, int importance) {
                WifiLockManager.this.mHandler.post(new Runnable(uid, importance) {
                    ...
                    public final void run() {
                        WifiLockManager.C04591.this.lambda$onUidImportance$0$WifiLockManager$1(this.f$1, this.f$2);
                    }
                });
            }
    ...
    public void handleScreenStateChanged(boolean screenOn) {
        ...
        boolean isMaxDtimInSuspend = true;
        if (canActivateLowLatencyLock(1)) {
            updateOpMode();
            setBlameLowLatencyWatchList(screenOn);
        }
        if (!screenOn && (ifaceName = this.mWifiNative.getClientInterfaceName()) != null) {
            if (this.mForceHiPerfMode || this.mFullHighPerfLocksAcquired > this.mFullHighPerfLocksReleased) {
                isMaxDtimInSuspend = false;
            }
            this.mWifiNative.setMaxDtimInSuspend(ifaceName, isMaxDtimInSuspend);
        }
    }
    ...
    public synchronized boolean updateOpMode() {
        ...
        } else if (!this.mClientModeImpl.setPowerSave(true)) {
        ...
        if (newLockMode != 0) {
            ...
            } else if (!this.mClientModeImpl.setPowerSave(false)) {
    ...
    private boolean setLowLatencyMode(boolean enabled) {
        ...
        if (lowLatencySupport == 1) {
            if (!this.mClientModeImpl.setLowLatencyMode(enabled)) {
                Slog.e(TAG, "Failed to set low latency mode");
                return false;
            } else if (!this.mClientModeImpl.setPowerSave(!enabled)) {
                Slog.e(TAG, "Failed to set power save mode");
                this.mClientModeImpl.setLowLatencyMode(!enabled);
                return false;
            }
        } else if (lowLatencySupport == 0 && !this.mClientModeImpl.setPowerSave(!enabled)) {
            Slog.e(TAG, "Failed to set power save mode");
            return false;
        }
        return true;
    ...
    public void setBlameLowLatencyUid(int uid, boolean shouldBlame) {
        ...
        if (shouldBlame) {
            try {
                ...
                StatsLog.write_non_chained(37, uid, (String) null, 1, 4);
            } catch (RemoteException e) {
            } catch (Throwable th) {
                Binder.restoreCallingIdentity(ident);
                throw th;
            ...
            StatsLog.write_non_chained(37, uid, (String) null, 0, 4);
        }
        Binder.restoreCallingIdentity(ident);
    }
