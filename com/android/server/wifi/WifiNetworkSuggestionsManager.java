...
public class WifiNetworkSuggestionsManager {
    ...
    public static class PerAppInfo {
        ...
        public int hashCode() {
            return Objects.hash(new Object[]{this.packageName, this.extNetworkSuggestions, Boolean.valueOf(this.hasUserApproved)});
        }
    }
    ...
    public static class ExtendedWifiNetworkSuggestion {
        ...
        public int hashCode() {
            return Objects.hash(new Object[]{this.wns});
        }
        ...
        public void onOpChanged(String op, String packageName) {
            WifiNetworkSuggestionsManager.this.mHandler.post(new Runnable(packageName, op) {
                ...
                public final void run() {
                    WifiNetworkSuggestionsManager.AppOpsChangedListener.this.mo3929xe5165a09(this.f$1, this.f$2);
                }
            });
        }

        /* renamed from: lambda$onOpChanged$0$WifiNetworkSuggestionsManager$AppOpsChangedListener */
        public /* synthetic */ void mo3929xe5165a09(String packageName, String op) {
    ...
    private Set<WifiNetworkSuggestion> convertToWnsSet(Collection<ExtendedWifiNetworkSuggestion> extNetworkSuggestions) {
        return (Set) extNetworkSuggestions.stream().collect(Collectors.mapping(C0348x414f0141.INSTANCE, Collectors.toSet()));
    }

    private boolean canAddNetworkSuggestionsMDM() {
        if (WifiMobileDeviceManager.getEnterprisePolicyEnabled(this.mContext, "content://com.sec.knox.provider2/WifiPolicy", "getAllowUserProfiles", new String[]{"false"}) == 0) {
            return false;
        }
        return true;
    }
    ...
    public int add(List<WifiNetworkSuggestion> networkSuggestions, int uid, String packageName) {
        ...
        if (networkSuggestions.isEmpty()) {
            ...
            return 0;
        } else if (!canAddNetworkSuggestionsMDM()) {
            Log.e(TAG, "Adding networkSuggestions blocked by MDM");
            return 1;
        } else {
            ...
            if (!Collections.disjoint(perAppInfo.extNetworkSuggestions, extNetworkSuggestions)) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (config.isEnterprise() && !this.mWifiKeyStore.updateNetworkKeys(config, (WifiConfiguration) null)) {
    ...
    public int remove(List<WifiNetworkSuggestion> networkSuggestions, int uid, String packageName) {
        if (this.mVerboseLoggingEnabled) {
    ...
    private List<Integer> getAllMaxSizes() {
        return (List) this.mActiveNetworkSuggestionsPerApp.values().stream().map(C0349xbedd3be4.INSTANCE).collect(Collectors.toList());
    }

    private PendingIntent getPrivateBroadcast(String action, String packageName, int uid) {
        return this.mFrameworkFacade.getBroadcast(this.mContext, 0, new Intent(action).setPackage("android").putExtra(EXTRA_PACKAGE_NAME, packageName).putExtra(EXTRA_UID, uid), 134217728);
    }
    ...
    private void sendUserApprovalNotification(String packageName, int uid) {
        Notification.Action userAllowAppNotificationAction = new Notification.Action.Builder((Icon) null, this.mResources.getText(17042738), getPrivateBroadcast(NOTIFICATION_USER_ALLOWED_APP_INTENT_ACTION, packageName, uid)).build();
        Notification.Action userDisallowAppNotificationAction = new Notification.Action.Builder((Icon) null, this.mResources.getText(17042739), getPrivateBroadcast(NOTIFICATION_USER_DISALLOWED_APP_INTENT_ACTION, packageName, uid)).build();
        CharSequence appName = getAppName(packageName);
        this.mNotificationManager.notify(51, new Notification.Builder(this.mContext, SystemNotificationChannels.NETWORK_STATUS).setSmallIcon(17304154).setTicker(this.mResources.getString(17042741)).setContentTitle(this.mResources.getString(17042741)).setContentText(this.mResources.getString(17042740, new Object[]{appName})).setDeleteIntent(getPrivateBroadcast(NOTIFICATION_USER_DISMISSED_INTENT_ACTION, packageName, uid)).setShowWhen(false).setLocalOnly(true).setColor(this.mResources.getColor(17170460, this.mContext.getTheme())).addAction(userAllowAppNotificationAction).addAction(userDisallowAppNotificationAction).build());
    ...
    public Set<WifiNetworkSuggestion> getNetworkSuggestionsForScanDetail(ScanDetail scanDetail) {
        ...
        Set<ExtendedWifiNetworkSuggestion> approvedExtNetworkSuggestions = (Set) extNetworkSuggestions.stream().filter(C0346xe0e7a368.INSTANCE).collect(Collectors.toSet());
    ...
    private Set<ExtendedWifiNetworkSuggestion> getNetworkSuggestionsForWifiConfiguration(WifiConfiguration wifiConfiguration, String bssid) {
        ...
        Set<ExtendedWifiNetworkSuggestion> approvedExtNetworkSuggestions = (Set) extNetworkSuggestions.stream().filter(C0344x1dd7040a.INSTANCE).collect(Collectors.toSet());
        if (approvedExtNetworkSuggestions.isEmpty()) {
            return null;
        }
        if (this.mVerboseLoggingEnabled) {
    ...
    private void handleConnectionSuccess(WifiConfiguration connectedNetwork, String connectedBssid) {
        ...
        if (matchingExtNetworkSuggestions != null && !matchingExtNetworkSuggestions.isEmpty()) {
            ...
            Set<ExtendedWifiNetworkSuggestion> matchingExtNetworkSuggestionsWithReqAppInteraction = (Set) matchingExtNetworkSuggestions.stream().filter(C0347xb17adf61.INSTANCE).collect(Collectors.toSet());
