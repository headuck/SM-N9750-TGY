...
public class WifiAwareClientState {
    ...
    public void onInterfaceAddressChange(byte[] mac) {
        Log.v(TAG, "onInterfaceAddressChange: mClientId=" + this.mClientId + ", mNotifyIdentityChange=" + this.mNotifyIdentityChange + ", mac=" + String.valueOf(HexEncoding.encode(mac)) + ", mLastDiscoveryInterfaceMac=" + String.valueOf(HexEncoding.encode(this.mLastDiscoveryInterfaceMac)));
        if (this.mNotifyIdentityChange && !Arrays.equals(mac, this.mLastDiscoveryInterfaceMac)) {
            try {
                boolean hasPermission = this.mWifiPermissionsUtil.checkCallersLocationPermission(this.mCallingPackage, this.mUid, true);
                Log.v(TAG, "hasPermission=" + hasPermission);
                this.mCallback.onIdentityChanged(hasPermission ? mac : ALL_ZERO_MAC);
    ...
    public void onClusterChange(int flag, byte[] mac, byte[] currentDiscoveryInterfaceMac) {
        Log.v(TAG, "onClusterChange: mClientId=" + this.mClientId + ", mNotifyIdentityChange=" + this.mNotifyIdentityChange + ", mac=" + String.valueOf(HexEncoding.encode(mac)) + ", currentDiscoveryInterfaceMac=" + String.valueOf(HexEncoding.encode(currentDiscoveryInterfaceMac)) + ", mLastDiscoveryInterfaceMac=" + String.valueOf(HexEncoding.encode(this.mLastDiscoveryInterfaceMac)));
        if (this.mNotifyIdentityChange && !Arrays.equals(currentDiscoveryInterfaceMac, this.mLastDiscoveryInterfaceMac)) {
            try {
                boolean hasPermission = this.mWifiPermissionsUtil.checkCallersLocationPermission(this.mCallingPackage, this.mUid, true);
                Log.v(TAG, "hasPermission=" + hasPermission);
                this.mCallback.onIdentityChanged(hasPermission ? currentDiscoveryInterfaceMac : ALL_ZERO_MAC);
            } catch (RemoteException e) {
                Log.w(TAG, "onIdentityChanged: RemoteException - ignored: " + e);
            }
        }
