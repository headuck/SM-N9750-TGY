...
public class WifiAwareNativeCallback extends IWifiNanIfaceEventCallback.Stub implements WifiAwareShellCommand.DelegatedShellCommand {
    ...
    public void eventDataPathRequest(NanDataPathRequestInd event) {
        if (this.mDbg) {
            Log.v(TAG, "eventDataPathRequest: discoverySessionId=" + event.discoverySessionId + ", peerDiscMacAddr=" + String.valueOf(HexEncoding.encode(event.peerDiscMacAddr)) + ", ndpInstanceId=" + event.ndpInstanceId + ", appInfo.size()=" + event.appInfo.size());
        }
        incrementCbCount(8);
    ...
    public void eventDataPathConfirm(NanDataPathConfirmInd event) {
        ...
        incrementCbCount(9);
        this.mWifiAwareStateManager.onDataPathConfirmNotification(event.ndpInstanceId, event.peerNdiMacAddr, event.dataPathSetupSuccess, event.status.status, convertArrayListToNativeByteArray(event.appInfo), (List<NanDataPathChannelInfo>) null);
    }
