...
public class OsuServerConnection {
    ...
    public void init(SSLContext tlsContext, TrustManagerImpl trustManagerImpl) {
        if (tlsContext != null) {
            try {
                ...
                tlsContext.init((KeyManager[]) null, new TrustManager[]{this.mTrustManager}, (SecureRandom) null);
    ...
    public boolean validateProvider(Map<String, String> friendlyNames) {
        ...
        for (Pair<Locale, String> identity : ServiceProviderVerifier.getProviderNames(this.mTrustManager.getProviderCert())) {
            if (identity.first != null && !TextUtils.isEmpty((CharSequence) identity.second) && TextUtils.equals((CharSequence) identity.second, friendlyNames.get(((Locale) identity.first).getISO3Language()))) {
                if (!this.mVerboseLoggingEnabled) {
                    return true;
                }
                Log.v(TAG, "OSU certificate is valid for " + ((Locale) identity.first).getISO3Language() + "/" + ((String) identity.second));
    ...
    public void lambda$connect$0$OsuServerConnection(URL url, Network network) {
        ...
        this.mUrl = url;
        try {
            URLConnection urlConnection = this.mNetwork.openConnection(this.mUrl);
            if (urlConnection instanceof HttpsURLConnection) {
                ...
                PasspointProvisioner.OsuServerCallbacks osuServerCallbacks3 = this.mOsuServerCallbacks;
                if (osuServerCallbacks3 != null) {
                    osuServerCallbacks3.onServerConnectionStatus(osuServerCallbacks3.getSessionId(), false);
                }
            }
        } catch (IOException e2) {
            Log.e(TAG, "Unable to establish a URL connection: " + e2);
            PasspointProvisioner.OsuServerCallbacks osuServerCallbacks4 = this.mOsuServerCallbacks;
            if (osuServerCallbacks4 != null) {
                osuServerCallbacks4.onServerConnectionStatus(osuServerCallbacks4.getSessionId(), false);
            }
        }
    }
    ...
    public void lambda$exchangeSoapMessage$1$OsuServerConnection(SoapSerializationEnvelope soapEnvelope) {
        ...
        if (this.mServiceConnection == null) {
            ...
            PasspointProvisioner.OsuServerCallbacks osuServerCallbacks = this.mOsuServerCallbacks;
            if (osuServerCallbacks != null) {
                osuServerCallbacks.onReceivedSoapMessage(osuServerCallbacks.getSessionId(), (SppResponseMessage) null);
                return;
            }
            return;
        }
        try {
            ...
            if (response == null) {
                ...
                if (this.mOsuServerCallbacks != null) {
                    this.mOsuServerCallbacks.onReceivedSoapMessage(this.mOsuServerCallbacks.getSessionId(), (SppResponseMessage) null);
                }
    ...
    public void lambda$retrieveTrustRootCerts$2$OsuServerConnection(Map<Integer, Map<String, byte[]>> trustCertsInfo) {
        ...
        for (Map.Entry<Integer, Map<String, byte[]>> certInfoPerType : trustCertsInfo.entrySet()) {
            ...
            Iterator it = certInfoPerType.getValue().entrySet().iterator();
            while (true) {
                if (!it.hasNext()) {
                    break;
                }
                Map.Entry<String, byte[]> certInfo = (Map.Entry) it.next();
                ...
                if (certificate == null) {
                    trustRootCertificates.clear();
                    break;
                } else if (!ServiceProviderVerifier.verifyCertFingerprint(certificate, certInfo.getValue())) {
                    trustRootCertificates.clear();
                    String certName = "";
    ...
    private class WFATrustManager implements X509TrustManager {
        ...
        public void checkServerTrusted(X509Certificate[] chain, String authType) throws CertificateException {
            ...
            try {
                ...
                Log.e(OsuServerConnection.TAG, "Certificate validation failure: " + e);
                int i = 0;
                for (X509Certificate cert : chain) {
                    Log.e(OsuServerConnection.TAG, "Cert " + i + " details: " + cert.getSubjectDN());
                    Log.e(OsuServerConnection.TAG, "Not before: " + cert.getNotBefore() + ", not after: " + cert.getNotAfter());
                    Log.e(OsuServerConnection.TAG, "Cert " + i + " issuer: " + cert.getIssuerDN());
                    i++;
                }
            }
