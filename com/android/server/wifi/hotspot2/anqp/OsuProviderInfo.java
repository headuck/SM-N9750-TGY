...
public class OsuProviderInfo {
    ...
    public /* synthetic */ void lambda$new$0$OsuProviderInfo(I18Name e) {
        String put = this.mFriendlyNames.put(e.getLocale().getLanguage(), e.getText());
    }

    public static OsuProviderInfo parse(ByteBuffer payload) throws ProtocolException {
        ByteBuffer byteBuffer = payload;
        int length = ((int) ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 2)) & 65535;
        if (length >= 9) {
            List<I18Name> friendlyNameList = parseI18Names(getSubBuffer(byteBuffer, ((int) ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 2)) & 65535));
            ...
            int availableIconLength = ((int) ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 2)) & 65535;
            ByteBuffer iconBuffer = getSubBuffer(byteBuffer, availableIconLength);
            ArrayList arrayList = new ArrayList();
            while (iconBuffer.hasRemaining()) {
                arrayList.add(IconInfo.parse(iconBuffer));
            }
            ArrayList arrayList2 = arrayList;
            int i = availableIconLength;
            return new OsuProviderInfo(friendlyNameList, serverUri, methodList, arrayList, ByteBufferReader.readStringWithByteLength(byteBuffer, StandardCharsets.UTF_8), parseI18Names(getSubBuffer(byteBuffer, ((int) ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 2)) & 65535)));
        }
