...
public class IntHistogram implements Iterable<Bucket> {
    ...
    public IntHistogram(int[] bucketBoundaries) {
        ...
        while (i < bucketBoundaries.length - 1) {
            ...
            if (cur < next) {
                ...
                throw new IllegalArgumentException(String.format("bucketBoundaries values must be strictly monotonically increasing, but value %d at index %d is greater or equal to value %d at index %d!", new Object[]{Integer.valueOf(cur), Integer.valueOf(i), Integer.valueOf(next), Integer.valueOf(i + 1)}));
            }
        }
    ...
    public Bucket getBucketByIndex(int bucketIndex) {
        ...
        return new Bucket(start, bucketKey == iArr.length ? Integer.MAX_VALUE : iArr[bucketKey], this.mBuckets.valueAt(bucketIndex));
    }
    ...
    public <T> T[] toProto(Class<T> protoClass, ProtobufConverter<T> converter) {
        T[] output = (Object[]) Array.newInstance(protoClass, this.mBuckets.size());
        int i = 0;
