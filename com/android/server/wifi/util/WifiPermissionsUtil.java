...
public class WifiPermissionsUtil {
    ...
    public boolean checkConfigOverridePermission(int uid) {
        try {
            ...
            this.mLog.err("Error checking for permission: %").mo2073r(e.getMessage()).flush();
            return false;
        }
    }
    ...
    public void enforceCanAccessScanResults(String pkgName, int uid) throws SecurityException {
        ...
        if (!checkNetworkSettingsPermission(uid) && !checkNetworkSetupWizardPermission(uid) && !checkNetworkManagedProvisioningPermission(uid) && !checkNetworkStackPermission(uid) && !checkMainlineNetworkStackPermission(uid) && !checkAccessSecuredInformationPermission(uid) && !checkDiagnosticsProviderPermission(uid)) {
    ...
    public void enforceCanAccessScanResultsForWifiScanner(String pkgName, int uid, boolean ignoreLocationSettings, boolean hideFromAppOps) throws SecurityException {
        ...
        if (!isLocationModeEnabled()) {
            if (ignoreLocationSettings) {
                ...
                wifiLog.mo2096w("Request from " + pkgName + " violated location settings");
            } else {
                throw new SecurityException("Location mode is disabled for the device");
            }
        }
    ...
    public boolean checkCanAccessWifiDirect(String pkgName, int uid, boolean needLocationModeEnabled) {
        try {
            checkPackage(uid, pkgName);
            if (checkNetworkSettingsPermission(uid) || checkSignatureForWifiDirect(pkgName)) {
                return true;
            }
    ...
    public boolean checkSystemAlertWindowPermission(int callingUid, String callingPackage) {
        ...
        if (mode == 3) {
            if (this.mWifiPermissionsWrapper.getUidPermission("android.permission.SYSTEM_ALERT_WINDOW", callingUid) == 0) {
                return true;
            }
            return false;
        } else if (mode == 0) {
    ...
    public boolean checkDiagnosticsProviderPermission(int uid) {
        return this.mWifiPermissionsWrapper.getUidPermission("com.samsung.permission.WIFI_DIAGNOSTICS_PROVIDER", uid) == 0;
    }

    public boolean checkFactoryTestPermission(int uid) {
        return this.mWifiPermissionsWrapper.getUidPermission("com.samsung.permission.WIFI_FACTORY_TEST", uid) == 0;
    }

    public boolean checkAccessSecuredInformationPermission(int uid) {
        return this.mWifiPermissionsWrapper.getUidPermission("com.samsung.permission.SEM_ACCESS_WIFI_SECURED_INFO", uid) == 0;
    }

    public boolean checkSignatureForWifiDirect(String pkgName) {
        try {
            if (this.mContext.getPackageManager().checkSignatures("android", pkgName) == 0) {
                return true;
            }
            return false;
        } catch (Exception e) {
            Slog.e(TAG, "exception in checkSignatureForWifiDirect : " + e);
            return false;
        }
    }
}
