...
public class IwcBigDataManager {
    ...
    public static final String FEATURE_MIWC = "MIWC";
    private static final String PACKAGE_NAME_SURVEY = "com.samsung.android.providers.context";
    private static final String TAG = "IwcBigDataManager";
    public final String APP_ID = "android.net.wifi";
    public final HashMap<String, IwcBigDataFeature> mBigDataFeatures;
    private final Context mContext;
    private SemHqmManager mSemHqmManager;

    public IwcBigDataManager(Context context) {
    ...
    private void initialize() {
        ...
        this.mBigDataFeatures.put(FEATURE_MIWC, new IwcBigDataMIWC());
    }
    ...
    public boolean addOrUpdateFeatureValue(String feature, String param, boolean value) {
        if (!this.mBigDataFeatures.containsKey(feature)) {
            return false;
        }
        this.mBigDataFeatures.get(feature).addOrUpdateValue(param, value);
    ...
    public IwcBigDataFeature getBigDataFeature(String feature) {
        if (this.mBigDataFeatures.containsKey(feature)) {
    ...
    public boolean clearAllFeatures() {
        ...
        for (IwcBigDataFeature feature : this.mBigDataFeatures.values()) {
            feature.resetData();
        }
        return false;
    }
    ...
    public void insertLog(String feature, long value) {
        String hittype;
        if (value == 3) {
            hittype = BaseBigDataItem.HIT_TYPE_ONCE_A_DAY;
        } else {
            hittype = BaseBigDataItem.HIT_TYPE_IMMEDIATLY;
        }
        if (this.mBigDataFeatures.containsKey(feature)) {
            ...
            if (this.mBigDataFeatures.get(feature).getIsDqaEnabled()) {
                if (this.mBigDataFeatures.get(feature).getDqaFeatureName() != null) {
                    sendHWParamToHQMwithAppId(this.mBigDataFeatures.get(feature).getDqaFeatureName(), hittype, extra);
                } else {
                    sendHWParamToHQMwithAppId(feature, hittype, extra);
                }
            }
    ...
    private void sendHWParamToHQMwithAppId(String feature, String hitType, String basicCustomDataSet) {
        String str = feature;
        if (this.mSemHqmManager == null) {
            ...
            this.mSemHqmManager.sendHWParamToHQMwithAppId(0, BaseBigDataItem.COMPONENT_ID, feature, hitType, wifiChipInfo.getCidInfo(), compManufacture, "", basicCustomDataSet, "", "android.net.wifi");
        }
    }
}
