...
public class Base64 {
    ...
    public static StringBuffer encode(byte[] data, int start, int len, StringBuffer buf) {
        ...
        int n = 0;
        while (i <= end) {
            int d = ((data[i] & 255) << SemWifiApSmartUtil.BLE_BATT_2) | ((data[i + 1] & 255) << 8) | (data[i + 2] & 255);
        ...
        if (i == (start + len) - 2) {
            int d2 = ((data[i] & 255) << SemWifiApSmartUtil.BLE_BATT_2) | ((data[i + 1] & 255) << 8);
            buf.append(charTab[(d2 >> 18) & 63]);
            buf.append(charTab[(d2 >> 12) & 63]);
            buf.append(charTab[(d2 >> 6) & 63]);
            buf.append("=");
        } else if (i == (start + len) - 1) {
            int d3 = (data[i] & 255) << SemWifiApSmartUtil.BLE_BATT_2;
